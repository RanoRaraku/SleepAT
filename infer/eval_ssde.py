...
def eval_ssde(ref:list, hyp:list, events:dict, thr:float = 2/3) -> np.ndarray:
    """
    ...
    D = np.zeros(shape=(reflen,hyplen),dtype=np.float32)
    ...
    tmp1 = np.zeros(shape=(reflen,hyplen),dtype=np.float32)
    ...
    tmp2 = np.zeros(shape=(reflen,hyplen),dtype=np.float32)
    for l in range(hyplen):
        col = tmp1[:,l]
        k = argmax(col)
        tmp2[k,l] = tmp1[k,l]
