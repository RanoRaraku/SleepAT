...
from sleepat.base.opts import SimpleDatasetOpts, PyClassOpts
from sleepat.io import read_scp, read_npy
from sleepat.feat import add_delta, apply_mvn, splice_frames
...
class SimpleDataset(Dataset):
    ...
    def __init__(self, data_dir:str, config:str=None, **kwargs):
        ...
        <splice_frames> ... whether to apply frame splicing (def:bool = True)
            <context_left> ... no. frames to left to splice onto center frame (def:int = 2)
            <context_right> ... no. frames to right to splice onto center frame (def:int = 2)
        ...
        **kwargs ... setting all above through kwargs
        """
        self.conf = SimpleDatasetOpts(config,**kwargs)
    ...
    def __getitem__(self, idx):
        ...
        """
        frame_idx = self.meta[idx]['example_idx']
        return {"target": as_tensor(self.targets[frame_idx]),
            "sample": as_tensor(self.feats[frame_idx])}
    ...
    def make_meta(self):
        ...
        for utt_id, flen in feats_to_len(self.targets_scp):
            for frame_idx in range(self.conf.context_left, flen - self.conf.context_right):
                self.meta[db_idx] = {'example_idx': frame_idx+frames_num,'utt_frame':frame_idx, 'utt_id': utt_id}
                db_idx += 1
            frames_num += flen
    ...
    def set_sample_size(self):
        """
        Set total number of samples for indexing.
        """
    ...
    def set_sample_dim(self):
        ...
        if self.conf.splice_frames:
            m *= (1+self.conf.context_left+self.conf.context_right)
    ...
    def make_examples(self):
        ...
        if not hasattr(self,'sample_size'):
            self.set_sample_size()
        if not hasattr(self,'sample_dim'):
            self.set_sample_dim()

        total = (self.sample_size + (self.conf.context_left + 
            self.conf.context_right)*len(self.utt2spk),)
        self.targets = np.empty(shape = total + self.target_dim, dtype=np.float32)
        self.feats = np.empty(shape= total + self.sample_dim, dtype=np.float32)
    ...
    def process_feats(self, feats, spk_id):
        ...
        if self.conf.splice_frames:
            feats = splice_frames(feats, context_left=self.conf.context_left,
                context_right=self.conf.context_right)
        if self.conf.apply_lda:
            feats = feats
